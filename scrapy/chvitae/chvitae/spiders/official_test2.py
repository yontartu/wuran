...
class OfficialSpider(CrawlSpider):
	...
	def parse(self, response):
		...
		for ix, link in enumerate(links): # follow links to activity pages
			...
			request = scrapy.Request(response.urljoin(link), callback=self.parse_activity)
	...
	def parse_activity(self, response):
		...
		if len(ncv_links) > 0: # if ncv list is NOT empty
			for i, official_url in enumerate(ncv_links): # loop through official links ('i' is the official counter)
				print('------Looping thru ncv_links. Processing links #', i)
				...
				yield scrapy.Request(response.urljoin(official_url), callback=self.get_official_name_and_title, meta={'loader':loader, 'i':i})
